...
async function determinarGanadorSeguro(esSuperpote) {
    ...
    if (esSuperpote) return Math.floor(Math.random() * 30);
...
app.post('/realizar-apuesta', async (req, res) => {
    ...
    try {
        ...
        if (!usuario || Number(usuario.saldo) < montoNum) return res.status(400).json({ error: "Saldo insuficiente" });
...
app.post('/solicitar-deposito', (req, res) => {
    ...
    const nuevaSol = db.ref('sistema/solicitudes/depositos').push({ userId, monto, referencia, estado: 'pendiente', timestamp: Date.now() });
...
app.post('/deshacer-apuesta', async (req, res) => {
    ...
    if (!apuesta) return res.json({ success: false, message: "No hay apuestas recientes." });
...
io.on('connection', (socket) => {
    ...
    socket.on('disconnect', () => {
        if(socket.userId) db.ref(`sistema/online/${socket.userId}`).remove();
    });
